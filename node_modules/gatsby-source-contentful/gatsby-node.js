...
exports.sourceNodes = async ({
  ...
  fetchActivity.end();
  const processingActivity = reporter.activityTimer(`Contentful: Proccess data (${sourceId})`, {
    parentSpan
  });
  ...
  function mergeSyncData(previous, current, deleted) {
    ...
    previous.forEach(e => !deleted.includes(e.sys.id) && entryMap.set(e.sys.id, e));
    current.forEach(e => !deleted.includes(e.sys.id) && entryMap.set(e.sys.id, e));
    return [...entryMap.values()];
  }

  const mergedSyncData = {
    entries: mergeSyncData(previousSyncData.entries, currentSyncData.entries, currentSyncData.deletedEntries.map(e => e.sys.id)),
    assets: mergeSyncData(previousSyncData.assets, currentSyncData.assets, currentSyncData.deletedAssets.map(e => e.sys.id))
  ...
  existingNodes.filter(n => _.includes(newOrUpdatedEntries, `${n.id}___${n.sys.type}`)).forEach(n => {
